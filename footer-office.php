        ...
        <?php
            $officeImages = array();
            if(!intval(get_theme_mod('office_numberbgimages_control_setting')) <= 0){
                for($i = 1; $i <= intval(get_theme_mod('office_numberbgimages_control_setting')); $i++){
                if(strlen(get_theme_mod('office_backstretch_img_' . $i)) != 0){
                    array_push($officeImages, get_theme_mod('office_backstretch_img_' . $i));
                }
                switch($i - 1){
                    case 0:
                    echo strlen($officeImages[$i]);
                    //if(strlen($officeImages[$i - 1]) == 0){array_push($officeImages, '' . WPMEDIA . '/bg-6.jpg');}
                    break;
                    case 1:
                    //if(strlen($officeImages[$i - 1]) == 0){array_push($officeImages, '' . WPMEDIA . '/bg-7.jpg');}
                    break;
                    default:
                    break;
                    }
                ...
                array_push($officeImages, WPMEDIA . '/bg-6.jpg');
                array_push($officeImages, WPMEDIA . '/bg-7.jpg');
            }
        ?>
        <!--Include and use Backstretch -->
        <script src="<?php print THEMEROOT?>/src/backstretch/jquery.backstretch.min.js"></script>
        <script> $("#header").backstretch([<?php for($i = 0; $i < count($officeImages); $i++){if(strlen($officeImages[$i]) > 0){echo $i == (count($officeImages) - 1) ?  "'$officeImages[$i]'" : "'$officeImages[$i]', ";}}?>], { duration: 4000, fade: 750 }); </script>
